...
describe("StakingEther", function () {
  ...
  async function deployStakingEther() {
    ...
    const stakingEther = await stakingEthContract.deploy({ value: ethers.parseEther("10") });
  ...
  describe("stake", function () {
    ...
    it("should revert with error if zero ether is passed", async function () {
      ...
      expect(
        stakingEther.connect(otherAccount).stake(30, { value: ethers.parseEther("0") })
    ...
    it("should revert with error if max duration is exceeded", async function () {
      ...
      expect(
        stakingEther.connect(otherAccount).stake(5.6863e6, { value: ethers.parseEther("2") })
      ).to.be.revertedWithCustomError(
        stakingEther,
    ...
    it("should create new stake for user", async function () {
      const { stakingEther, owner, otherAccount } = await loadFixture(
        deployStakingEther
      );

      const duration = 60;
      await stakingEther.connect(otherAccount).stake(duration, { value: ethers.parseEther("0.7") });
    ...
    it("should revert with custom when maximum number of stakes for a user is reached", async function () {
      ...
      await stakingEther.connect(otherAccount).stake(15, { value: ethers.parseEther("0.23") });
      await stakingEther.connect(otherAccount).stake(23, { value: ethers.parseEther("0.23") });
      await stakingEther.connect(otherAccount).stake(56, { value: ethers.parseEther("0.23") });
      ...
      expect(
        stakingEther.connect(otherAccount).stake(62, { value: ethers.parseEther("0.25") })
      ).to.be.revertedWithCustomError(
        stakingEther,
    ...
    it("should emit an event when the stake is created", async function () {
      ...
      await expect(
        stakingEther.connect(otherAccount).stake(200, { value: ethers.parseEther("0.5") })
  ...
  describe("withdraw", function () {
    ...
    it("should revert with error if stake is already withdrawn", async function () {
      ...
      await stakingEther.connect(otherAccount).stake(10, { value: ethers.parseEther("0.2") });
